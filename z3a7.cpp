...
usage (void)
{
    std::cout << "Usage: lzwtree in_file -o out_file -in/-pre/-po" << std::endl;
}
...
main (int argc, char *argv[])
{
    ...
    std::string cmlarg = *argv;
        if(cmlarg.compare("-in"))
        binFa.kiir_in(kiFile);

        else if(cmlarg.compare("-pre"))
        binFa.kiir_pre(kiFile);

        else if(cmlarg.compare("-po"))
