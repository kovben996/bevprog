...
private:
    ...
    void kiir_pos (Csomopont * elem, std::ostream & os)
    {
        if (elem != NULL)
        {
            ++melyseg;
...
usage (void)
{
    std::cout << "Usage: lzwtree in_file -o out_file -in / -pre / -pos" << std::endl;
}
...
main (int argc, char *argv[])
{
    ...
    // a kiírás szerint ./lzwtree in_file -o out_file -in / -pre / -pos alakra kell mennie, ez 4 db arg:
    if (argc != 5)
    {
    ...
    std::string cmlarg = argv[1];
    
    if (cmlarg.compare("-in") == 0)
    {
        binFa.kiir_in(kiFile);
    }
    else if (cmlarg.compare("-pre") == 0)
    {
        binFa.kiir_pre(kiFile);
    }
    else if (cmlarg.compare("-pos") == 0)
    {
        binFa.kiir_pos(kiFile);
    }
